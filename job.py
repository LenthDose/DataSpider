...
def get_onepage(url, page_index):
    ...
    job = js_dict['engine_jds']  # list类型,job[0]为dict类型
    ...
    # print(df)
    if page_index == 1:
        df.to_csv('./guangzhou.csv', index=None, header=True, encoding='utf_8_sig')
    else:
        df.to_csv('./guangzhou.csv', mode='a', index=None, header=False, encoding='utf_8_sig')


if __name__ == '__main__':
        url = 'https://search.51job.com/list/000000,000000,0000,00,9,99,java,2,{}.html?lang=c&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&ord_field=0&dibiaoid=0&line=&welfare='.format(65)
